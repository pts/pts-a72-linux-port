...
HUMBUG:	CALLW	CLOSE
	MOV	AX,4C02H
	call dossys_exit  ; Exit with failure.
FUNC_NOTHER:	TEST	EA32_VAR_PLUS(FUNC,0),AX
	JZ	NOTH0
	CALLW	WRM
	MOV	AL,20H
	STOSB
...
PARANX:	POPR	SI
	...
	CLC
	RETW
FUNC_PROCF:	MOV	AX,EA32_VAR_PLUS(ORIGIN,0)
...
ADDEX0:	lea di, [bx+si]
	...
	STOSB
	RETW
FUNC_CALF:	MOV	AX,EA32_VAR_PLUS(INCLEV,0)
	TEST	AX,AX
	JZ	CALF0
	CLD
	DEC	AX
	...
	CLC
	RETW
FUNC_INCF:	INC	WORD EA32_VAR_PLUS(INCLEV,0)
	MOV	DI,SI
	CALLW	CALF
	JC	INCF0
	XCHG	DI,SI
...
FUNC_DECF:	CALLW	CALF
	...
	CLC
	RETW
FUNC_WLST:	MOV EA32_VAR_PLUS(STK,0), ESP
	PUSHW	AX
	PUSHW	DX
	PUSHW	BX
	PUSHW	SI
...
WTITL5:	INC	WORD EA32_VAR_PLUS(LSTLIN,0)
	...
	CLC
	RETW
FUNC_READ:	MOV EA32_VAR_PLUS(STK,0), ESP
	CALLW	CALF
	JC	READ1
...
READ8:	MOV	DX,4
	...
	MOV	AL,0FH
	JMP	FUNC_SCREAM  ; Tail call.
FUNC_WRITE:	MOV EA32_VAR_PLUS(STK,0), ESP
	MOV	AL,EA32_VAR_PLUS(FUNC,0)
	TEST	AL,AL
	JS	WRITE5
	TEST	AL,10H
...
WRITE5:	CLC
	RETW
FUNC_CLOSE:	CALLW	CALF
	JC	NOIN
	MOV	AH,3EH
	MOV	BX,EA_SI_PLUS(0)
	call dossys_close  ; Close file.
NOIN:	TEST	WORD EA32_VAR_PLUS(FUNC,0),4040H
	JZ	FUNC_NOLST  ; Tail call.
	TEST	WORD EA32_VAR_PLUS(FUNC,0),8080H
	JNZ	FUNC_NOLST  ; Tail call.
	MOV	AH,3EH
	MOV	BX,EA32_VAR_PLUS(LSTHDL,0)
	call dossys_close  ; Close file.
	; Fall through to tail call.
FUNC_NOLST:	TEST	WORD EA32_VAR_PLUS(FUNC,0),1010H
	JZ	NOOUT
	MOV	AH,3EH
	MOV	BX,EA32_VAR_PLUS(OUTHDL,0)
	call dossys_close  ; Close file.
NOOUT:	RETW
FUNC_OPEN:	MOV	SI,INFN
	CALLW	RFN2
	CALLW	INCF
	MOV	AL,0BH
	JC	FUNC_SCREAM  ; Tail call.
...
SCREM:  FIX_CX_AT_LOOP_TARGET
	...
	LOOP	SCREM
SCRAM:	CALLW	WRM
FUNC_PRUNT:	MOV	DX,EA32_VAR_PLUS(TEMP,0)
	MOV	AX,0A0DH
	STOSW
	MOV	CX,DI
	SUB	CX,DX
...
P6S:	MOV	EA32_VAR_PLUS(PAGLEN,0),DX
	CLC
	RETW
FUNC_SYMLST:	TEST	BYTE EA32_VAR_PLUS(FUNC,0),40H  ; This code is part of /d (disassembly).
	JNZ	SYMLS0
	RETW
SYMLS0:	CLD
	PUSHW	SI
...
SYMLS6:	DEC	CX
	JNZ	SYMLS4
FUNC_SYMLS7:	CMP	DI,EA32_VAR_PLUS(TEMP,0)
	JBE	SYMLS8
	DEC	DI
	DEC	DI
	MOV	AX,0A0DH
...
LBL2:	ADD	SI,BX
	...
	JNZ	LBL2S  ; Finally fixed the bug (`INC SI' below was incorrectly assumed to be 1 byte, now we use LBL2S).
	INC	SI
LBL2S:	RETW
LBL3:	ADD	DI,CX
	INC	DI
...
ERR3:	MOV	AL,3
	JMP	FAIL
FUNC_SETDFB:	MOV	DI,EA32_VAR_PLUS(BINBUF,0)
	XOR	CX,CX
SETD1:	CALLW	CC
	JZ	ERR8
	CMP	AL,3FH
...
SETIN3:	INC	BX
	...
	CMP	AL,3BH
	JZ	FUNC_SETEND  ; Tail call.
	CMP	AL,21H
	JNC	SETIN3
FUNC_SETEND:	MOV	AL,EA32_VAR_PLUS(OPCODE,0)
...
SETDFS:	CALLW	RI
	...
	CLC
	RETW
FUNC_SETEQU:	MOV	AL,EA32_VAR_PLUS(FUNC,0)
	TEST	AL,3
	JZ	SETEQ0
	CALLW	RI
	MOV	DI,BP
...
FUNC_ASMCMD:	XOR	BYTE EA32_VAR_PLUS(FLAGS,0),20H
	PUSHW	DX
	CALLW	SL
	POPR	DI
	JNZ	SW2
...
G0BH1:	MOV	AL,EA32_VAR_PLUS(ARGS,3)
	...
	JZ	FUNC_G0  ; Tail call.
	CMP	AL,10
	JNZ	FUNC_G0  ; Tail call.
G0BH2:	OR	BYTE EA32_VAR_PLUS(OPCODE,0),8
FUNC_G0:	RETW
FUNC_D0:	MOV	AX,EA32_VAR_PLUS(PC,0)
	SHR	AL,1
	SBB	AL,AL
	AND	AL,20H
	MOV	EA32_VAR_PLUS(FLAGS,0),AL
...
D6SEG:	MOV	AL,3EH
	CALLW	GSPR2
	XOR	CX,CX
	JMP	FUNC_G0AH  ; Tail call.
FUNC_D8:	MOV	DI,EA32_VAR_PLUS(TEMP,0)
D8L:	CALLW	CC
	JZ	D9X	;D8P
	CMP	AL,27H
	JZ	D8Q
...
;	SUB	BX,SI
;	JMP	FAIL
FUNC_D8F:	MOV EA32_VAR_PLUS(STK,0), ESP
FUNC_RI:	MOV	DL,24H
	JMP	FUNC_RE  ; Tail call.
FUNC_D9:	XOR	DX,DX
	CALLW	CC
	JZ	D9X
	CALLW	RI
	MOV	DX,AX
D9X:	JMP	FUNC_SETEND  ; Tail call.
DISASM:	CLD
	PUSHW	SI
	LODSB
	XOR	AH,AH
...
WMN2:	LODSB
	...
	POPR	SI
	RETW
FUNC_RELBD:	CMP	BYTE EA32_VAR_PLUS(OPCODE,0),0EBH
	JNZ	NOTJMP
	MOV	CL,20
	MOV	DX,D8086+2
	CALLW	WMN
...
;	DEC	DI
;RELBNM:	CBW
FUNC_WHAS:	TEST	AX,AX
	JNS	FUNC_WHA  ; Tail call.
	NEG	AX
...
FUNC_SINGD:	DEC	DI
	RETW
FUNC_SPREFD:	CALLW	PSRD
	MOV	AL,3AH
	STOSB
	RETW
FUNC_ACCID:	XOR	AL,AL
	CALLW	WREG
	JMP	G03DT
FUNC_PSRD:	MOV	AL,EA32_VAR_PLUS(OPCODE,0)
FUNC_WSR:	SHR	AL,1
	SHR	AL,1
	AND	AX,6
	ADD	AX,REGS+20H
...
FUNC_SMD:	MOV	AL,10H
	...
	STOSB
	POPR	AX
	JMP	FUNC_WDISP  ; Tail call.
FUNC_ACCRD:	MOV	AX,EA32_VAR_PLUS(REGS,10h)
	STOSW
	MOV	AL,2CH
	STOSB
FUNC_REGWD:	MOV	AL,10H
	MOV	EA32_VAR_PLUS(WADJ,0),AL
	MOV	AL,EA32_VAR_PLUS(OPCODE,0)
	OR	AL,0C0H
	JMP	FUNC_WDISP  ; Tail call.
FUNC_RIMMD:	MOV	AL,EA32_VAR_PLUS(OPCODE,0)
	PUSHW	SI
	AND	AX,0FH
	SHL	AL,1
	PUSHW	AX
...
FUNC_AAMD:	LODSB
	...
	CALLW	WHA
AAMD0:	RETW
FUNC_ESCD:	MOV	AL,EA32_VAR_PLUS(OPCODE,0)
	MOV	AH,EA_SI_PLUS(0)
	MOV	CL,3
	ROR	AH,CL
	AND	AX,707H
...
FUNC_INTD:	LODSB
	XOR	AH,AH
	JMP	FUNC_WHA  ; Tail call.
FUNC_INT3D:	MOV	AL,33H
	STOSB
	RETW
FUNC_RETFD:	MOV	CL,0AH
	MOV	DX,D8086+2
	CALLW	WMN
	TEST	BYTE EA32_VAR_PLUS(OPCODE,0),1
	JZ	FUNC_RETD  ; Tail call.
	DEC	DI
	RETW
FUNC_RETD:	LODSW
	JMP	FUNC_WHA  ; Tail call.
FUNC_ADIOD:	MOV	AX,EA32_VAR_PLUS(WADJ,0)
	ADD	AX,REGS
	PUSHW	SI
	MOV	SI,AX
	LODSW
...
FUNC_ACCMD:	MOV	AL,6
	JMP	RMD1
FUNC_RWMD:	MOV	BYTE EA32_VAR_PLUS(WADJ,0),10H
FUNC_RMD:	LODSB
RMD1:	PUSHW	AX
	CALLW	WREG
	MOV	AL,2CH
...
FUNC_G03D:	MOV	CX,_G03D
	...
	JZ	G03DT
	RETW
FUNC_G01D:	MOV	CX,_G01D
	CALLW	GRPD
FUNC_RMID:	CALLW	WPTR
G03DT:	MOV	AL,2CH
	STOSB
	LODSW
	TEST	BYTE EA32_VAR_PLUS(WADJ,0),0FFH
...
G03DB:	CBW
	DEC	SI
G03DW:	JMP	FUNC_WHA  ; Tail call.
FUNC_G04D:	MOV	CX,_G04D
	CALLW	GRPD
	CMP	AL,7
	JZ	FUNC_INVD  ; Tail call.
	CMP	AL,5
	JZ	G04DF
	CMP	AL,4
	JZ	FUNC_DISPD  ; Tail call.
	CMP	AL,3
	JZ	G04DF
	CMP	AL,2
	JZ	FUNC_DISPD  ; Tail call.
	JMP	FUNC_WPTR  ; Tail call.
G04DF:	MOV	CL,0AH
	MOV	DX,D8086+2
	CALLW	WMN
FUNC_DISPD:	LODSB
	JMP	FUNC_WDISP  ; Tail call.
FUNC_INVD:	MOV	AX,4244H
	STOSW
	MOV	AX,9
	STOSB
	MOV	AL,EA32_VAR_PLUS(OPCODE,0)
...
FUNCT_IHDL:  ; Function call table.
	DW	FUNC_G0,FUNC_G1,FUNC_G2,FUNC_G3
	DW	FUNC_G4,FUNC_G5,FUNC_G6,FUNC_G7
	DW	FUNC_G8,FUNC_G9,FUNC_G0AH,FUNC_G0BH
	DW	FUNC_G0CH,FUNC_G0DH,FUNC_G0EH,FUNC_G0FH
FUNCT_AHDL:  ; Function call table.
	DW	FUNC_D0,FUNC_SETEQU,FUNC_SETEND
	DW	FUNC_SETORG,FUNC_SETDFB
	DW	FUNC_D5,FUNC_D6,FUNC_SETINC
	DW	FUNC_D8,FUNC_D9
FUNCT_DHDL:  ; Function call table.
	DW	FUNC_MRD,FUNC_RMD,FUNC_ACCID
	DW	FUNC_PSRD,FUNC_SPREFD,FUNC_SINGD
	DW	FUNC_REGWD,FUNC_INVD,FUNC_RELBD
	DW	FUNC_G01D,FUNC_G03D,FUNC_G04D
	DW	FUNC_MSD,FUNC_SMD,FUNC_DISPD
	DW	FUNC_ACCRD,FUNC_SEGOFD,FUNC_ACCMD
	DW	FUNC_MACCD,FUNC_RIMMD,FUNC_RETD
	DW	FUNC_RWMD,FUNC_RMID,FUNC_RETFD
	DW	FUNC_INT3D,FUNC_INTD,FUNC_G02D
	DW	FUNC_AAMD,FUNC_ESCD,FUNC_IBAPD
	DW	FUNC_RELWD,FUNC_ADIOD
...
FUNCT_ADIR:  ; Function call table.
	DW	FUNC_P2,FUNC_P3,FUNC_P4,FUNC_P5,FUNC_P6
